class Fly {
  ...
  int food;
  Fly() {
    location = new PVector(random(backgroundWidth*2), random(height));
  ...
  void update(PVector target) {
    ...
    // damage
    if ((location.x > 355) && (location.x < 365) 
      && (location.y > 495) && (location.y < 510) && ecoSystem.electricGrass.isActive == true) {
      stroke(255);
      strokeWeight(3);
